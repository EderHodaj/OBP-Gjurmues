...
export default function EditableCell({ value, field, rowId, onSave, isHighlighted, display }) {
  ...
  return (
    <td className={`ec ${isHighlighted ? 'ec-hi' : ''} ${editing ? 'ec-active' : ''}`} onClick={!editing ? startEdit : undefined} title="Klikoni për të redaktuar">
      {editing ? (
        <input ref={inputRef} type={isNum ? 'number' : 'text'} value={draft} min={isNum ? '0' : undefined}
        ...
        <span className={`ec-disp ${isNum ? 'ec-num' : ''}`}>{display ?? (value ?? '')}<span className="ec-hint" aria-hidden="true">✎</span></span>
      )}
    </td>
  );
}
